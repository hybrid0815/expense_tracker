...
## Key
어떤 리스트 위젯이 아이템 위젯들을 포함하고 있다고 가정하고 
아이템들의 위치가 바뀌면 첵크 박스 같은 경우 이전 상태를 유지한다.
이는 플러터가 성능 최적화를 하기위해 같은 타입의 아이템 같은 경우 순서가 바뀌더라도
이전 상태를 유지 한다.
이를 방지 하기 위해 `Kye` 값을 아이템 마다 부여하여 순서가 바뀌면 바뀌 순서로 레퍼런싱을 
다시 하게 한다.
```dart
key: ValueKey(item.someValue)
key: ObjectKey(item)
```
